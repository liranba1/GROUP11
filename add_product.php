...
if(isset($_POST['submit']))
{
	...
	if(strlen((string)$barcode)>15 or empty($barcode))
		echo "<script>alert('Error product barcode len must be between 1 to 15'); window.location.href='add_product.php'</script>";
  else if(ctype_alpha($barcode)==TRUE)
  	echo "<script>alert('Error product barcode must be numbers'); window.location.href='add_product.php'</script>";
  else if(strlen($name)>30 or empty($name))
		echo "<script>alert('Error product name len must be between 1 to 30'); window.location.href='add_product.php'</script>";
	else if(strlen($image)>50 or empty($image))
		echo "<script>alert('Error image name len must be between 1 to 50'); window.location.href='add_product.php'</script>";
	else if(strlen((string)$price)>10 or empty($price))
		echo "<script>alert('Error product price len must be between 1 to 10'); window.location.href='add_product.php'</script>";
	else if(strlen((string)$quantity)>10 or empty($quantity))
		echo "<script>alert('Error product quantity len must be between 1 to 10'); window.location.href='add_product.php'</script>";
	else
	{
	if($sz=='All')
	{
