...
<a href="shippment_reports.php"><p align="right">Back to shippments search</p></a>
</center>
<?php if (!isset($_POST['submit'])) {?>
	<br><br><br><br><br>
	<center>
	<form action="shippment_reports.php" method="post">
	<table border=1>
	<tr>
...
<?php 
	...
	$d=$_POST['day'];
	if(empty($y))
		echo "<script type='text/javascript'>alert('You must Enter atleast year!');window.location.href='shipmments_reports.php'</script>";
	if(!empty($y) and !empty($m))
	{
		$query="SELECT * from shippments WHERE year='$y' and month='$m' and INNER JOIN orders ON shippments.id=orders.id";
	}
	...
	{
		$query="SELECT * from shippments WHERE year='$y' and month='$m' INNER JOIN orders ON shippments.id=orders.id";
	}
